...
#       Add the sdlc scaffolding to an EXISTING project without clobbering
#       it. Copies .github/ + docs scaffolding additively (existing files
#       are kept unless --force), NEVER overwrites the project's CLAUDE.md
#       (only creates one from the stack template if absent), and writes
#       .sdlc-template-version.
#
...
Subcommands:
  ...
  adopt <project-path> --stack=<...> [--force]
      Add the scaffolding to an existing project without clobbering it.
      Additive; never overwrites the project's CLAUDE.md.
...
Examples:
  ...
  $(basename "$0") adopt ~/Projects/existing-app --stack=typescript
...
cmd_adopt() {
  ...
  local version_file="$target/.sdlc-template-version"
  if [[ -e "$version_file" && "$force" -eq 0 ]]; then
    die "$target/.sdlc-template-version exists — already bootstrapped/adopted (use --force to re-stamp)"
  fi

  require_clean_template_repo

  echo "Adopting sdlc scaffolding into $target (stack=$stack)"
  echo "(additive: existing files are kept unless --force; CLAUDE.md is never overwritten)"
  echo

  # Additively copy the .github/ + docs/ scaffolding. Unlike `init`, existing
  # project files are preserved by default (reported KEPT), so a project that
  # already has its own .github/ keeps it; only missing files are ADDED.
  local f proj
  while IFS= read -r f; do
    [[ -n "$f" ]] || continue
    proj="$target/$f"
  ...
  if [[ -e "$target/CLAUDE.md" ]]; then
    printf '%-9s %s\n' "KEPT" "CLAUDE.md (existing project file preserved)"
  else
    cp "$stack_template" "$target/CLAUDE.md"
    printf '%-9s %s\n' "ADDED" "CLAUDE.md (from $(basename "$stack_template"))"
  fi
...
EOF
  ...
  echo
  echo "Adopted sdlc_template @ ${sha:0:7} into $target (stack=$stack)"
  echo "Next: 'sync.sh check $target' to see drift; your CLAUDE.md was left untouched —"
  echo "      add a Work Tracking section pointing at the new docs/ scaffolding if you want forks to use it."
}
